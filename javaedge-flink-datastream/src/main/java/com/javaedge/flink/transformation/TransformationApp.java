package com.javaedge.flink.transformation;

import com.javaedge.flink.source.AccessSource;
    ...
    public static void connect(StreamExecutionEnvironment env) {
        ...
        SingleOutputStreamOperator<Tuple2<String, Access>> stream2new = stream2.map(new MapFunction<Access, Tuple2<String, Access>>() {
            ...
            public Tuple2<String, Access> map(Access value) throws Exception {
                return Tuple2.of("JavaEdge", value);
            }
        });
...
//        stream1.union(stream2).print();
    ...
    public static void richMap(StreamExecutionEnvironment env) {
        ...
        SingleOutputStreamOperator<Access> mapStream = source.map(new JavaEdgeMapFunction());
        mapStream.print();
         ...
         * wc： socket
         *
         * 进来的数据：JavaEdge,JavaEdge,flink   JavaEdge,spark,spark
         *
         *
         * wc需求分析：
         * 1) 读进来数据
         * 2) 按照指定分隔符进行拆分  JavaEdge JavaEdge flink JavaEdge spark spark
         * 3) 为每个单词赋上一个出现次数为1的值 (JavaEdge,1) (JavaEdge,1) ...
         * 4) 按照单词进行keyBy
         * 5) 分组求和
         *
         */
...
//        }).sum("traffic").print();
        ...
        /**
         * 进来是一行行的数据： JavaEdge,JavaEdge,flink   JavaEdge,spark,spark
         * 需求：
         * 1) 把一行数据按照逗号进行分割
         * 2) 过滤掉JavaEdge
         */
